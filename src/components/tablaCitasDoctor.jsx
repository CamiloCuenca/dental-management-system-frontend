...
const CitasDoctor = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      const errorMsg =
        "No se pudo obtener el ID del doctor. Por favor, inicie sesión nuevamente.";
      setError(errorMsg);
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const citasFormateadas = response.data.map((cita) => {
          const fechaCitaLuxon = DateTime.fromISO(cita.fechaHora, {
            zone: "America/Bogota",
          });

          return {
            ...cita,
            fechaHora: fechaCitaLuxon.toFormat("dd/MM/yyyy HH:mm"),
            fechaParaComparacion: fechaCitaLuxon.toFormat("yyyy-MM-dd"),
            fecha: fechaCitaLuxon.toLocaleString(DateTime.DATE_MED),
          };
        });
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-400 text-sm mt-2">
                Las citas aparecerán aquí cuando sean asignadas
              </p>
            </div>
          ) : (
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    <span
                      className={`${getEstadoClass(
                        cita.estado
                      )} font-medium shadow px-3 py-1 inline-block`}
                    >
                      {cita.estado}
                    </span>
